package com.yourcompany.catalogo.datalake
...
class raw_producto_mes(huemulBigDataGov: huemul_BigDataGovernance, Control: huemul_Control) extends huemul_DataLake(huemulBigDataGov, Control) with Serializable  {
   ...
   CurrentSetting.FileName = "PRODUCTO_{{YYYY}}{{MM}}.txt"
   ...
   //Forma detallada
   CurrentSetting.DataSchemaConf.AddColumns("producto_id", "producto_id", StringType, "codigo del producto",0,9)
   CurrentSetting.DataSchemaConf.AddColumns("producto_nombre", "producto_nombre", StringType, "Nombre del producto",10,48)
  ...
  */
  def open(Alias: String, ControlParent: huemul_Control, ano: Integer, mes: Integer, dia: Integer, hora: Integer, min: Integer, seg: Integer): Boolean = {
    ...
    try { 
      ...
      //****VALIDACION DQ*****
      //**********************
      control.NewStep("Valida que cantidad de registros esté entre 2 y 100")    
      //validacion cantidad de filas
      val validanumfilas = this.DataFramehuemul.DQ_NumRowsInterval(this, 2, 100)      
